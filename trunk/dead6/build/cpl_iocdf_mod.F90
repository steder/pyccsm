!===============================================================================
! CVS: $Id: cpl_iocdf_mod.F90,v 1.1.1.1 2005/02/03 22:29:02 steder Exp $
! CVS: $Source: /home/cvsroot/steder/pyCPL/dead6build/cpl_iocdf_mod.F90,v $
! CVS: $Name:  $
!===============================================================================
!BOP ===========================================================================
!
...
!
! !DESCRIPTION:
!    This module creates a file and writes bundle aVect data to the file
!    in machine-independent netCDF format.  This module is intended to support 
!    the history sub-system of cpl6.
!
! !REMARKS:
...
!
! !DESCRIPTION:
!    Create a new netCDF file with no content other than global attributes.
!
! !REMARKS:    
!
! !REVISION HISTORY:
...
!------------------------------------------------------------------------------
   ...
   !----------------------------------------------------------------------------
   str   = "cpl6 data file -- $RCSfile: cpl_iocdf_mod.F90,v $ $Revision: 1.1.1.1 $"
   ...
   str   ='cpl6: CCSM coupler component (CVS Tag $Name:  $)'
...
!
! !REMARKS:    
!    If the input time is not already in the time dimension, it is appended to 
!    the end of the time dimension.  This algorithm assumes that any time value 
...
!
!    If fName is not present, fid must be a valid netCDF file ID for an open
!    netCDF file.  This is presumably the normal mode of operation as it can
!    minimize the opening & closing of a file.
!
!    If fName is present, the named file is opened, data is appended, and the
!    file is closed.  In this case, the input fid is not used, although it's
!    value will be corrupted.  This mode is handy for "one-off" debug files.
...
!EOP
   ...
   type(cpl_mct_aVect) :: gGrid    ! global/gathered bundle data
   ...
   real(R8),parameter  :: time_eps = .01/86400.0  ! time epsilon = .01 seconds
   ...
   type(cpl_mct_string) :: mctStr  ! temporary mct/mpeu string var
...
!------------------------------------------------------------------------------
   ...
   !----------------------------------------------------------------------------
   call cpl_mct_aVect_gather(bun%data,gData,bun%dom%gsMap,pid0,cpl_comm_comp,rcode)
   call cpl_mct_aVect_gather(bun%dom%lGrid,gGrid,bun%dom%gsMap,pid0,cpl_comm_comp,rcode)
   ...
   if ( rcode /= NF_NOERR ) then ! assume error means dim has not be defined yet
      ...
      do n=1,cpl_mct_aVect_nRAttr(gGrid)

         call cpl_mct_list_get(mctStr,n,gGrid%rList)
         aName = cpl_mct_string_toChar(mctStr)
         ...
         call cpl_mct_string_clean(mctStr)
      ...
      npts  = cpl_mct_aVect_lsize (gGrid)
      allocate(XXX(npts))
      ...
      do n=1,cpl_mct_aVect_nRAttr(gGrid)
         ...
         !--- put the data ---
         k = cpl_mct_aVect_indexRA(gGrid,trim(aName),perrWith=subName)
         XXX = gGrid%rAttr(k,:)
   ...
   do n=1,cpl_mct_aVect_nRAttr(bun%data)

      !--- construct netCDF var name ---
      call cpl_mct_list_get(mctStr,n,bun%data%rList)
      aName = cpl_mct_string_toChar(mctStr)
      call cpl_mct_string_clean(mctStr)
      bName = bun%name
      ...
      if ( rcode == NF_NOERR ) then
         ...
         if ( present(dateS) .and. (present(dateE)) ) then
            ...
            timeS  = float(eDay) + sec/86400.0
            call shr_date_getEDay(dateE,eDay,sec)
            timeE  = float(eDay) + sec/86400.0
            if ( timeS /= timeE ) then
               str = "time: mean"
   ...
   do n=1,cpl_mct_aVect_nRAttr(bun%data)
      ...
      !--- put the data ---
      k = cpl_mct_aVect_indexRA(gData,trim(aName),perrWith=subName)
      XXX = gData%rAttr(k,:)
