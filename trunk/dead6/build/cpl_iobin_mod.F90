!===============================================================================
! CVS: $Id: cpl_iobin_mod.F90,v 1.1.1.1 2005/02/03 22:29:02 steder Exp $
! CVS: $Source: /home/cvsroot/steder/pyCPL/dead6build/cpl_iobin_mod.F90,v $
! CVS: $Name:  $
!===============================================================================
!BOP ===========================================================================
!
...
!
! !DESCRIPTION:
!    This module creates, writes-to, and reads bundle aVect data to/from a file
!    in machine-dependent binary format.  This module is intended to support 
!    the restart sub-system of cpl6.
!
! !REMARKS:
!    Typically this module would be used to support the restart file subsystem
!    of cpl6, but in fact it could be used to create binary data files for any
...
!
! !DESCRIPTION:
!    Create a new file in machine dependent binary format.
!
! !REVISION HISTORY:
!    2002-Nov-08 - B. Kauffman, initial version
!
...
!EOP
   ...
   character(CL),parameter :: cvsID = &
   & "@(#) CVS: $RCSfile: cpl_iobin_mod.F90,v $ $Revision: 1.1.1.1 $"
...
!------------------------------------------------------------------------------
   ...
   format   = "format-str256:name:F90_date_and_time:caseDesc:cvsId"
...
!EOP
   ...
   character(CL) :: cvsId    ! CVS Id    string
   character(CL) :: str      ! generic   string
...
!
! !DESCRIPTION:
!    Gather bundle data, then append data to an existing binary data file.
!
! !REMARKS:    
!    This module writes a bundle's aVect data to a file.  Associated domain data
!    is not written, but this functionality could be added, if desired.
!    The file format utilizes an extensible, self-describing data format.
!
!    fid must be a valid fortran unit number for an open file.
!
! !REVISION HISTORY:
!    2002-Nov-08 - B. Kauffman, initial version
!
...
!------------------------------------------------------------------------------
   ...
   do n=1,cpl_mct_aVect_nRAttr(bun%data)
      ...
      aName   = cpl_mct_string_toChar(mctStr) ! aVect var name
...
!
! !DESCRIPTION:
!    Reads bundle data from a file, then scatters the bundle.
!
! !REVISION HISTORY:
!    2002-Nov-08 - B. Kauffman, initial version
!
...
!-------------------------------------------------------------------------------
   ...
   if (cpl_comm_comp_pid == pid0) then
      ...
      call cpl_mct_aVect_init(gData,bun%data,npts) ! it aVect, copy rList
...
!!!      vName = trim(bName) // "_" // trim(aName) // "_" // trim(dSuffix) ! old
   ...
   call cpl_mct_aVect_scatter(gData,bun%data,bun%dom%gsMap,pid0,cpl_comm_comp,rcode)
...
!
! !DESCRIPTION:
!    Append real array data from a file.
!
! !REMARKS:
...
!
! !DESCRIPTION:
!    Read real array data from a file.
!
! !REMARKS:
!    This routine is run on root PE only.
!
...
!
! !DESCRIPTION:
!    Get the data cDate (yyyymmdd) and seconds from an open file.
!
! !REMARKS
