...
! !INPUT PARAMETERS: 
      ...
      integer, optional,      intent(in)    :: lsize
      ! Bounds:
      integer, dimension(1) :: inLbound, outLbound
...
! 19Oct01 - J.W. Larson <larson@mcs.anl.gov> - initial (slow) 
!           prototype.
! 2/15/06 - M. Steder <steder@gmail.com> Modified import / export routines to handle different lowerbounds.
!
!EOP ___________________________________________________________________
  ...
  !end do
  ! New version for Arrays with variable Lower Bounds
  inLbound = lbound(inVect)
  outLbound = lbound(aV%iAttr(index,:))
  do n=inLbound(1),(inLbound(1)+mysize-1)
     aV%iAttr(index,outLbound(1)+n) = inVect(n)
  enddo
...
!EOP ___________________________________________________________________
  ...
  ! New version for Arrays with variable Lower Bounds
  inLbound = lbound(inVect)
  outLbound = lbound(aV%rAttr(index,:))
  do n=inLbound(1),(inLbound(1)+mysize-1)
     aV%rAttr(index,outLbound(1)+n) = inVect(n)
  enddo
