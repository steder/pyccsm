!===============================================================================
! CVS: $Id: dead.F90,v 1.6 2003/04/21 01:07:33 tcraig Exp $
! CVS: $Source: /fs/cgd/csm/models/CVS.REPOS/dead/dead6/dead.F90,v $
...
! !USES:
   ...
   !steder: for now just call cpl_comm_init directly
   use cpl_comm_mod
   use cpl_fields_mod
...
!-------------------------------------------------------------------------------
   ...
   ! Call Specific Routines, leave StdOut Alone so we see what's going on:
   call shr_msg_chdir   (trim(str)) ! changes cwd
   !call shr_msg_chStdOut(trim(str)) ! open units 5 & 6 to named files
   call shr_msg_chStdIn (trim(str)) ! open units 5 & 6 to named files
   call shr_sys_flush(6)
   ...
   seg_len        =  -9999
   ncpl           =  -9999
! 1, 2, 903 are valid values for decomp_type
   decomp_type    =  -9999
   simTime        =  -9999.
...
!0.0                  !  simul time proxy (secs): time between cpl comms
   ...
   write(6,*) 'checking coupling frequency...'
   call shr_sys_flush(6)
! steder: Is this a problem?
   ncpl_v1 = 0
   do n = 1,max_con
      write(6,*) 'checking coupling frequency (n=',n,')...'
      if (ncpl(n) /= -9999) then
   ...
   !steder: For now call cpl_comm_init directly
   write(6,*) 'Calling CPL_COMM_INIT_WO_MPH(',myModelName,'local_comm )'
   call shr_sys_flush(6)
   call cpl_comm_init_wo_mph( myModelName, local_comm )
   
   !write(6,*) 'Calling CPL_COMM_INIT(',myModelName,'local_comm )'
   !call shr_sys_flush(6)
   ...
   if (myModelName == cpl_fields_lndname) then
      ...
      call cpl_interface_contractRecv(cpl_fields_cplname,contractR(2), &
                                      irbuf_g,rbuf_g)
      ! Debugging statements -- Msteder
      write(6,*) "LND Contract Recv: Initial latitude data:"
      !do n = 1,(nx(1)*ny(1))
      !   write(6,*) cpl_fields_grid_lat, n, rbuf_g( n, cpl_fields_grid_lat )
      !enddo
      
      do n=1,(nx(1)*ny(1))
         j = (n-1) / nx(1) + 1
         i = mod(n-1,nx(1)) + 1
         write(6,*) n,i,j,rbuf_g(n, cpl_fields_c2lg_alat)
      enddo

      write(6,*) "LND Contract Recv: End Initial Latitude data!"
      ! End debugging statements -- Msteder
      deallocate(rbuf_g)
   endif

   write(6,*) '\tcpl_interface_contractInit, contractS(1)'
   call cpl_interface_contractInit(contractS(1),myModelName,cpl_fields_cplname, &
        fields_m2c_list,isbuf,gbuf)
   write(6,*) '\tcpl_interface_contractInit, contractR(1)'
   ...
   if (myModelName == cpl_fields_lndname) then
      ...
      write(6,*) '\tcpl_interface_contractInit, contractS(2)'
      write(6,*) '\t',decomp_type(2),nxg(2),nyg(2), &
      nx(2),ny(2),npesx(2),npesy(2),mype
      call cpl_interface_contractInit(contractS(2),myModelName, &
        cpl_fields_cplname,fields_r2c_list,isbuf_r,gbuf_r)
      write(6,*) '\tFinished runoff contract'
   endif

   !----------------------------------------------------------------------------
   !  get initial message from coupler with cday, etc.
   !----------------------------------------------------------------------------
   write(6,*) '\tget initial infobuffer from coupler (recv)'
   call cpl_interface_infobufRecv(cpl_fields_cplname,irbuf)
   write(6,*) '\treceived initial infobuffer'
   ...
   ! msteder
   write(6,*) 'Returning from subroutine dead_init'
...
!-------------------------------------------------------------------------------
   ...
   loop: do
      ...
      n1=n1+1
      do n2=1,ncpl(1)
         !write(6,*) trim(myModelName),': Starting hour',n2,ncpl(1),'...'
         call shr_timer_start(timer02)
         !if (dbug >= 3) then 
         write(6,*) trim(myModelName),': receiving contract...'
         !endif
         call cpl_interface_contractRecv(cpl_fields_cplname,contractR(1), &
                                         irbuf,rbuf)
         !if (dbug >= 3 ) then
         write(6,*) trim(myModelName),': received contract!'
         !end if
...
!                    phase shift is 60 deg (pi/3)
         ...
         write(6,*) trim(myModelName),': sending contract...'
         call cpl_interface_contractSend(cpl_fields_cplname,contractS(1),isbuf,sbuf)
         write(6,*) trim(myModelName),': sent contract!'
         !--------------------------------------------------------------
